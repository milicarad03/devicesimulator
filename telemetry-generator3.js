...
function createTelemetryGenerator(schema) {
  ...
  function peakFluctuation() {
    Object.keys(fieldDefinitions).forEach((path) => {
      ...
      if (path.toLowerCase().includes("temp") || path.toLowerCase().includes("pressure")) {
        state[path] += randomBetween(0.03 * range, 0.07 * range);
      }
    });
  }
  ...
  function build() {
    ...
    Object.keys(fieldDefinitions).forEach((path) => {
      ...
      if (def.enum) {
        ...
        // 1. Primeni multipleOf ako postoji
        if (def.multipleOf) {
          val = Math.round(val / def.multipleOf) * def.multipleOf;
        }

        // 2. Ograniči na min/max
        const min = def.minimum ?? -Infinity;
        const max = def.maximum ?? Infinity;
        val = Math.min(Math.max(val, min), max);

        // 3. Ažuriraj state (da simulator pamti korigovanu vrednost)
        state[path] = val; 

        // 4. Formiraj finalnu vrednost
        const finalValue = def.type === "integer" ? Math.round(val) : Number(val.toFixed(10));
        setDeepValue(payload, path, finalValue);
      } else if (def.type === "boolean") {
        setDeepValue(payload, path, state[path]);
      } else if (def.type === "string") {
       
        if (path.toLowerCase().includes("firmware") || path.toLowerCase().includes("version")) {
