...
if (!DEVICE_ARG || !MODEL_ARG || !VERSION_ARG) {
  console.error(" Usage: node sim.js <device> <model> <version>");
  process.exit(1);
}
...
if (/^\d+$/.test(DEVICE_ARG)) {
   DEVICE_FOLDER = `device-${DEVICE_ARG}`;
} else {
  DEVICE_FOLDER = DEVICE_ARG; // npr "device4"
}
...
const SCHEMA_FILE = path.join(
  ...
  "schema",
  MODEL_ARG,
  `${VERSION_ARG}.schema.json`
);

if (!fs.existsSync(SCHEMA_FILE)) {
  throw new Error(`[SCHEMA] Missing: ${SCHEMA_FILE}`);
}

console.log("[DEVICE] Using schema:", SCHEMA_FILE);
...
console.log("[DEVICE] Using certs folder:", DEVICE_CERT_DIR);
