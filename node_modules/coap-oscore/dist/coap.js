...
exports.isEmptyAckOrRst = isEmptyAckOrRst;
exports.splitOptions = splitOptions;
exports.mergeOptions = mergeOptions;
exports.serializeOptionsOnly = serializeOptionsOnly;
    ...
    while (offset < buf.length) {
        ...
        if (delta === 13) {
            if (offset >= buf.length)
                throw new Error('Option delta truncated');
            delta = buf[offset] + 13;
            offset++;
        }
        else if (delta === 14) {
        ...
        if (length === 13) {
            if (offset >= buf.length)
                throw new Error('Option length truncated');
...
function serialize(pkt) {
    ...
    if (pkt.payload.length > 0) {
        parts.push(Buffer.from([constants_1.PAYLOAD_MARKER]));
        parts.push(pkt.payload);
    }
    return Buffer.concat(parts);
}
function encodeOption(delta, value) {
    const parts = [];
    let nibbleDelta;
    const extDelta = [];
    if (delta < 13) {
...
function isRequest(pkt) {
    // Code class is upper 3 bits (code >> 5). Class 0 = requests.
    return (pkt.code >> 5) === 0 && pkt.code !== constants_1.CODE_EMPTY;
}
function isEmptyAckOrRst(pkt) {
    return (pkt.type === constants_1.TYPE_ACK || pkt.type === constants_1.TYPE_RST) && pkt.code === constants_1.CODE_EMPTY;
}
function splitOptions(options) {
    const eOptions = [];
    const uOptions = [];
    for (const opt of options) {
        if (opt.number === constants_1.OPTION_OSCORE) {
            // OSCORE option itself goes into U-options (it's the signaling option)
            continue;
        }
        if ((0, constants_1.isClassE)(opt.number)) {
            eOptions.push(opt);
        }
        else {
            uOptions.push(opt);
...
function mergeOptions(uOptions, eOptions) {
    // Merge and sort by option number, filtering out the OSCORE option
    const all = [...uOptions, ...eOptions].filter(o => o.number !== constants_1.OPTION_OSCORE);
    all.sort((a, b) => a.number - b.number);
    return all;
}
function serializeOptionsOnly(options) {
