...
class Agent extends events_1.EventEmitter {
    constructor(opts) {
        ...
        this._parameters = (0, parameters_1.createParameters)(opts.parameters);
        if (opts.type == null) {
            opts.type = 'udp4';
        }
        if (opts.socket != null) {
        ...
        this._oscoreOnly = (_a = opts.oscoreOnly) !== null && _a !== void 0 ? _a : false;
        this._init(opts.socket);
    }
    addOscoreContext(host, port, instance) {
        const key = `${host.toLowerCase()}:${port}`;
        this._oscoreContexts.set(key, instance);
    }
    removeOscoreContext(host, port) {
        this._oscoreContexts.delete(`${host.toLowerCase()}:${port}`);
    }
    _getOscoreContext(host, port) {
    ...
    _init(socket) {
        ...
        this._sock.on('message', (msg, rsinfo) => {
            ...
            let outerPacket;
            try {
                outerPacket = (0, coap_packet_1.parse)(msg);
            }
            catch (_b) {
                return;
            }
            if (!(0, oscore_helpers_1.hasOscoreOption)(outerPacket)) {
                ...
                const token = (_a = outerPacket.token) === null || _a === void 0 ? void 0 : _a.toString('hex');
            ...
            oscoreCtx.decode(msg)
                ...
                let packet;
                try {
                    packet = (0, coap_packet_1.parse)(decoded);
                }
                catch (_a) {
                    return;
                }
                ...
                if (this._sock != null) {
                    const oscoreRsinfo = Object.assign(Object.assign({}, rsinfo), { oscore: true });
                ...
                if (token != null && token.length > 0) {
                    ...
                    if (req != null) {
                        req.sender.reset();
                        req.emit('error', new Error(`OSCORE decode failed: ${(_b = err === null || err === void 0 ? void 0 : err.message) !== null && _b !== void 0 ? _b : 'unknown error'}`));
                    }
                }
            });
        });
    ...
    _handlePlainMessage(msg, rsinfo) {
        let packet;
        try {
    ...
    _handle(packet, rsinfo, outSocket) {
        ...
        if (packet.code === '4.01' && req != null && this._getOscoreContext(rsinfo.address, rsinfo.port) != null) {
            const echoOpt = (0, helpers_1.getOption)(packet.options, '252');
            if (echoOpt != null) {
                // Limit Echo retries to prevent infinite loops from malicious servers
                const retryCount = (_a = req._echoRetries) !== null && _a !== void 0 ? _a : 0;
                if (retryCount >= 1) {
                    // Max retries exceeded — deliver the 4.01 to the application
                    // (fall through to normal response handling below)
                }
                else {
                    const retryUrl = Object.assign(Object.assign({}, req.url), { token: req._packet.token });
                    ...
                    retryReq._echoRetries = retryCount + 1;
        ...
        if (req.response != null) {
            if (req.response instanceof observe_read_stream_1.default) {
        ...
        if (observe && packet.code !== '4.04') {
            const observeStream = new observe_read_stream_1.default(packet, rsinfo, outSocket);
    ...
    request(url) {
        ...
        const port = (_c = url.port) !== null && _c !== void 0 ? _c : this._parameters.coapPort;
        const req = new outgoing_message_1.default({}, (req, packet) => {
            ...
            if (block1Buff != null) {
                ...
                const block1State = (0, block_1.parseBlockOption)(block1Buff);
                ...
                if (packet.payload && packet.payload.length > blockSizeBytes) {
                    req.segmentedSender = new segmentation_1.SegmentedTransmission(block1State.size, req, packet);
        ...
        req.sender = new retry_send_1.default(this._sock, port, host, url.retrySend, this._parameters);
        const oscoreCtx = this._getOscoreContext(host !== null && host !== void 0 ? host : '', port);
        if (oscoreCtx == null && this._oscoreOnly) {
            const hostStr = host !== null && host !== void 0 ? host : '';
            throw new Error('No OSCORE context for ' + hostStr + (hostStr !== '' ? ':' : '') + port);
        }
        if (oscoreCtx != null) {
            const originalSend = req.sender.send.bind(req.sender);
            req.sender.send = (message, avoidBackoff) => {
