...
const DEVICE_ID = config.deviceId || 
                  config.serial || 
                  (config.messages && config.messages[0] && config.messages[0].deviceId) || 
                  (config.messages && config.messages[0] && config.messages[0].serial);
...
function sendTelemetry() {
  ...
  const rawMessage = config.messages[currentIndex];

  client.publish(TELEMETRY_TOPIC, JSON.stringify(rawMessage), { qos: 1 });

  console.log("[RAW TELEMETRY SENT]", JSON.stringify(rawMessage, null, 2));
